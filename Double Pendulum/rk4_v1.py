...
def runge_kutta_coupled(coupled_funcs , initial_state , tf , ti , n_iter = 1000) :
    """
       Solve system of coupled ordinary differential equations (ODEs) using the Runge-Kutta 4th order method.

       Parameters:
       - coupled_funcs: Function representing the system of coupled ODEs.
       - initial_state: Initial values of the dependent variables.
       - tf: Final time.
       - ti: Initial time.
       - n_steps: Number of time steps (default is 1000).
    ...
    """
    h = (tf - ti) / n_iter  # Calculates the step size
    x_space = np.zeros((initial_state.size , n_iter + 1))  # Placeholder for dependent variables
    x_space[: , 0] = initial_state  # Setting initial condition
    t_space = np.linspace(ti , tf , num = n_iter + 1)  # Time values

    for i in tqdm(range(n_iter) , desc = "Simulation Progress") :
        x_iter , t_iter = x_space[: , i] , t_space[i]
        k1 = h * coupled_funcs(x_iter , t_iter)
        k2 = h * coupled_funcs(x_iter + 0.5 * k1 , t_iter + 0.5 * h)
        k3 = h * coupled_funcs(x_iter + 0.5 * k2 , t_iter + 0.5 * h)
        k4 = h * coupled_funcs(x_iter + k3 , t_iter + h)
        x_space[: , i + 1] = x_iter + (1 / 6) * (k1 + 2 * k2 + 2 * k3 + k4)
...
def runge_kutta_vector(F_vector , tf , args = () , V_0 = np.array([0 , 0]) , t0 = 0 , dt = 1000) :
    """
